...
document.addEventListener('copy', (event) => {
  ...
  if (target.classList.contains('js-clone-url-http') || target.classList.contains('js-clone-url-ssh')) {
    ...
    let cloneUrl = target.getAttribute('value');
    let newCloneUrl = cloneUrl.startsWith('git clone ') ? cloneUrl : 'git clone ' + cloneUrl;
    event.clipboardData.setData('text/plain', newCloneUrl);
  }
});
